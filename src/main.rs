...
async fn main() -> std::io::Result<()> {
    ...
    HttpServer::new(|| {
        App::new()
            ...
            .service(
                web::resource("/api/posts/bySection").route(web::get().to(handlers::posts::get)),
            )
            .service(
                web::resource("/api/customers/{id}/followings/posts")
                    .route(web::get().to(handlers::posts::customers_followings_posts)),
            )
            .service(
                web::resource("/api/customers/{id}/posts")
                    .route(web::get().to(handlers::posts::customers_posts)),
            )
            .service(
                web::resource("/api/customers/{id}").route(web::get().to(handlers::customers::get)),
            )

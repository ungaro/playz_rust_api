...
) -> Vec<PostObject> 
{
    ...
    let gameprofilesql = format!("SELECT
    gameprofile.*, 
	game.*
    FROM
    gameprofile
    ...
    WHERE
    gameprofile.customer  IN ({})",customer_ids_str);
    ...
    for x in 0..gameprofiles.len() {    
        ...
        gameprofiles_hashmap.entry(gameprofiles[x].customer)
  ...
  let mut likedmessages_index = Vec::new();
    ...
    for x in 0..lposts.len() { 
        let mut tempcustomerobject:CustomerObject = if customers_hashmap.contains_key(&lposts[x].customer.unwrap()) {
        let tco = customers_hashmap.get(&lposts[x].customer.unwrap()).unwrap().clone().clone();       
        let mut gameprofile_customer = vec![];
        if gameprofiles_hashmap.contains_key(&lposts[x].customer.unwrap()) {
            gameprofile_customer=gameprofiles_hashmap.get(&lposts[x].customer.unwrap()).unwrap().clone().clone();
        }
            ...
            // SOMEHOW RUST DOESNT LIKE FUNCTIONS WITHOUT ELSE RETURNING
            // SO THIS IS SAME AS IF
    ...
    if postassets_hashmap.contains_key(&lposts[x].id) {
        output[x].assets.push(postassets_hashmap.get(&lposts[x].id).unwrap().clone().clone());
    }
...
pub async fn get(
    ...
    let cache_url = format!("cache:{}:/api/posts/bySection?section={}",  customer_id,section_id);
       ...
       // println!("parsedjson_from_cache__{:?}",parsedjson);        
        ...
        let mysql_pool = mysql_pool_handler(pool.clone())?;
            ...
            let limit = "20";
                    ...
                    let sql = format!("SELECT p.* FROM post as p
                    WHERE
                        p.createdAt < NOW() 
                    AND p.isReady = 1
            ...
            AND p.section = {}
            GROUP BY p.id
            ORDER BY p.id DESC LIMIT {}, {}",customer_id,customer_id,section_id,cursor,limit);
...
let limit = "20";
        ...
        let sql = format!("SELECT  * FROM post AS p
        INNER JOIN userconnection AS uc ON uc.with = p.customer
     WHERE uc.who = {} AND p.isReady = 1
     AND p.createdAt < NOW()
     GROUP BY p.id
     ORDER BY p.id DESC LIMIT {}, {}",customer_id,cursor,limit);
